...
describe('DiscountService', () => {
    ...
    test('should return filter response on successful fetch', async () => {
        (fetch as jest.MockedFunction<typeof fetch>).mockResolvedValueOnce({
            ok: true,
            json: async () => mockResponse,
        } as Response);

        const result = await FilterService.getFilteredList('test-bearer-token', 'param');
    ...
    test('should throw an error on fetch failure', async () => {
        ...
        await expect(FilterService.getFilteredList('test-bearer-token', 'param')).rejects.toThrow(
            mockError
        );
    });
});
